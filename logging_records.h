...
How actively we perform experiments based on how much power we have (one frequency for each type of experiment, frequency of detumbling (in attempts per 24 hours))

Only one uplink struct because it will be so small so we don't care about missing fields (i.e. missing TLE)
acaste42@calpoly.edu
...
/* Beacon:
    ...
    Warnings of strange sensor values (temp, attitude, is_in_eclipse, battery level 7-day low, high, avg watermark) (1 byte + 3 bytes for lo,hi,avg)
    ? highly compressed IMU data from the last experiment ?
    ? some experiment success criteria for each stored experiment ?
    ? compressed time-series CSS or mag values for secondary experiment verification ?
*/
