...
int testFuncID_HDD()
{
    ...
    vi_print("Returning test function id. \r\n");
    return 1;
}
...
{
    /*
    //const float MAX_START_DUTY = 10;  // previous max duty to trigger
calibration
    //const float MAX_DUTY = 10;  // targeted current max duty (should be no
higher than 10 for 2ms pulses)
    //const float MIN_DUTY = 5;  // targeted current min duty (should be no
lower than 5 for 1ms pulses) const int PERIOD_uS = 20000;  // period is
microseconds (5% duty -> min (1ms pulse), 10% duty -> max (2ms pulse))
    ...
    if (0) {
            ...
            printMsg("Calibration completed; 3 seconds until calibration
concludes \r\n."); delay_ms(3000); printMsg("Continuing. \r\n"); } else {
