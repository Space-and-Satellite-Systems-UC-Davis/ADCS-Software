...
const dutyType MAX_START_DUTY = duty_to_throttle(100);  // previous max duty to trigger calibration
const dutyType SLIP_DUTY = duty_to_throttle(75);  // last valid duty that will not cause any slipping
const dutyType MAX_DUTY = duty_to_throttle(100);  // targeted current max duty (should be no higher than 10 for 2ms pulses)
const dutyType MID_DUTY = duty_to_throttle(50);
const dutyType MIN_DUTY = duty_to_throttle(0);  // targeted current min duty (should be no lower than 5 for 1ms pulses)
