...
void hddRamp(const vi_sensor channel, const uint8_t TARGET_DUTY, int8_t doPrint) {
	if (doPrint) { vi_print("Ramping to %u. \r\n", TARGET_DUTY); }
	const uint8_t DUTY_STEP = 1;
	const uint32_t vi_delay_ms = 50;  // time between ramps

	// start at the middle duty + some base speed, then slowly increase
	uint8_t currDuty = vi_hdd_status(channel) + DUTY_STEP;
	if (doPrint) { vi_print("Taking first duty step to %u. \r\n", currDuty); }
	...
	if (doPrint) { vi_print("Ramped to %u. \r\n", TARGET_DUTY); }
}

void hddDrive(const vi_sensor channel, const dutyType TARGET_DUTY, int8_t doPrint) {
	if (TARGET_DUTY <= SLIP_DUTY) {
		if (doPrint) { vi_print("Drove to %u without slip handling.\r\n", TARGET_DUTY); }
		vi_hdd_command(channel, TARGET_DUTY);
		return;
	}

	// if we would slip going straight to target, ramp instead
	if (doPrint) { vi_print("Driving to %u with slip handling.\r\n", TARGET_DUTY); }
	...
	if (doPrint) { vi_print("Delaying for %u with currDuty %u\r\n", delayTimeMs, currDuty); }
